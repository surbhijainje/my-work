...
public class jacksondatabinder {
	@BeforeClass
  ...
  @Test
  public void post_validation() throws JsonProcessingException {
	 ...
	 // mainobj2.put("topping", jsonArray2);
		  ...
		  //we have to import objectmapper class from jackson databinder(com.fasterxml.jackson.databind.ObjectMapper) not from restassured.
		 ObjectMapper om=new ObjectMapper();
		
		 
		String mainobjectstr = om.writeValueAsString(mainobj3);   ///serialization(converting object into json format returning into string format)
			
		
		  RestAssured.given().body(mainobjectstr).when().log().all().post("/post").then();
		  
		  //deserialization
		  Response deserializer = RestAssured.given().body(mainobj3).when().log().all().post("/post").then().extract().response();
		 
	  System.out.println(deserializer.asString());
  }
}
